...
export class SessionService {
  ...
  signup(username:string, password:string):Observable<any>{
    return this.http.post(`${this.BASEURL}/api/signup`, {username,password}, this.options)
      .map(res => res.json())
  ...
  logout():Observable<any>{
    return this.http.get(`${this.BASEURL}/api/auth-login`,this.options)
      .map(res => res.json())
  ...
  isLoggedIn():Observable<any> {
    return this.http.get(`${this.BASEURL}/api/isLoggedIn`,this.options)
      .map(res => res.json())
      .map(this.configureUser(true))
      .catch(this.handleError);
  }
